#!/usr/bin/env bash
# Build one release APK for a single ABI.
#
...
#   ./scripts/fdroid_build.sh android-arm64 --build-name 0.0.1 --version-code-base 1
#
...
#                       packages/guar/android/app/build.gradle.kts (10*N + ABI).
#
...
parse_pubspec_version() {
  local raw
  raw="$(awk '/^version:/ { print $2; exit }' "${APP_DIR}/pubspec.yaml")"
  raw="${raw%\"}"
  raw="${raw#\"}"
  raw="${raw%\'}"
  raw="${raw#\'}"
...
# Lockfile lives at the pub workspace root, not in packages/guar.
cd "${ROOT}"
flutter pub get --enforce-lockfile
# Strip a non-reproducible build ID from libdartjni.so when the jni package is
# present. Same patch as Obtainium (ImranR98/Obtainium#2977). Idempotent so
# the per-ABI loop does not stack --build-id=none. Empty app may not pull jni
# yet; skip rather than fail until a plugin adds it.
shopt -s nullglob
jni_cmakes=("${PUB_CACHE}/hosted/"*/jni-*/src/CMakeLists.txt)
shopt -u nullglob
if [[ ${#jni_cmakes[@]} -eq 0 ]]; then
  echo "No jni CMakeLists.txt under PUB_CACHE=${PUB_CACHE}; skipping libdartjni.so build-id patch"
else
