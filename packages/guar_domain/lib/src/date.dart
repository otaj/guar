...
BeanDate addDays(BeanDate date, int days) {
  final DateTime native = DateTime.utc(date.year, date.month, date.day).add(Duration(days: days));
  return BeanDate(year: native.year, month: native.month, day: native.day);
}

BeanDate addDelta(BeanDate date, DateDelta delta) {
  int year = date.year + delta.years;
  int month = date.month + delta.months;
  while (month > 12) {
    year += 1;
    month -= 12;
  }
