...
void main() {
  test('costs-and-prices fixture lots', () {
    ...
    expect(ledger.accounts.map((AccountInventory e) => e.account.name).toList(), <String>[
      'Assets:NZ:Cash',
      'Assets:Shares:IBM',
      'Assets:UK:Cash',
    ]);

    final Inventory ibm = ledger.accounts
        .singleWhere((AccountInventory e) => e.account.name == 'Assets:Shares:IBM')
        .inventory;
    expect(ibm.length, 3);
    expect(ibm.currencyUnits(Currency(name: 'IBM')), amount('11', 'IBM'));
    expect(ibm.positions.map((Position p) => (p.units, p.cost)).toSet(), <(Amount, Cost)>{
