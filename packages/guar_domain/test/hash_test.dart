...
void main() {
  final source = Origin.source(BeanLocation(linenoBegin: 1, linenoEnd: 1));
  final elsewhere = Origin.source(BeanLocation(linenoBegin: 9, linenoEnd: 9));
  final date = BeanDate(year: 2024, month: 1, day: 1);
  final checking = account('Assets:Checking', AccountType.assets);

  Directive open({Origin? origin, Meta meta = const Meta(), String name = 'Assets:Checking'}) {
    return Directive(
  ...
  test('same date and body yield the same hash across origins and meta', () {
    ...
    final right = open(
      origin: elsewhere,
      meta: Meta(
        entries: [MetaEntry(key: 'filename', value: const MetaValue.text('other.beancount'))],
      ),
    );
  ...
  test('copyWith rematerializes after a body change and keeps hash when only meta changes', () {
    final original = open();
    final renamed = original.copyWith(body: DirectiveBody.open(account: account('Assets:Savings', AccountType.assets)));
    expect(renamed.hash, isNot(original.hash));
    expect(renamed.hash, open(name: 'Assets:Savings').hash);

    final annotated = original.copyWith(
      meta: Meta(
        entries: [MetaEntry(key: 'note', value: const MetaValue.text('x'))],
      ),
    );
  ...
  test('transaction postings participate in the hash', () {
    Directive txn(List<Posting> postings) {
      return Directive(
        origin: source,
        date: date,
