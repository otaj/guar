...
void assertParseTarget(String query, Expr expected) {
  final parsed = Query().parse(query);
  expect(parsed, isA<SelectStatement>());
  final statement = parsed as SelectStatement;
  final targets = statement.targets as List<Target>;
...
void main() {
  group('SELECT', () {
    ...
    test('columns and aliases', () {
      assertParse('SELECT date;', select([const Target(expression: Expr.column('date'))]));
      assertParse(
        'SELECT date, account',
        select([const Target(expression: Expr.column('date')), const Target(expression: Expr.column('account'))]),
      );
      assertParse('SELECT date as xdate;', select([const Target(expression: Expr.column('date'), name: 'xdate')]));
      assertParse(
    ...
    test('literals', () {
      ...
      assertParseTarget('SELECT (1, );', const Expr.constant([1]));
      assertParseTarget('SELECT (1, 2);', const Expr.constant([1, 2]));
      assertParseTarget('SELECT (1, 2, );', const Expr.constant([1, 2]));
      assertParseTarget("SELECT ('x', 'y', 'z');", const Expr.constant(['x', 'y', 'z']));
    ...
    test('attributes subscripts and quantified', () {
      ...
      assertParseTarget("SELECT a IN ('x', 'y');", const Expr.in_(Expr.column('a'), Expr.constant(['x', 'y'])));
      assertParseTarget(
        'SELECT a = any((1, 2));',
        const Expr.any(left: Expr.column('a'), op: '=', right: Expr.constant([1, 2])),
      );
      assertParseTarget(
        'SELECT a = all((1, 2));',
        const Expr.all(left: Expr.column('a'), op: '=', right: Expr.constant([1, 2])),
      );
    });
    ...
    test('boolean expressions', () {
      assertParseTarget('SELECT a AND b;', const Expr.and([Expr.column('a'), Expr.column('b')]));
      assertParseTarget(
        'SELECT a AND b AND c;',
        const Expr.and([Expr.column('a'), Expr.column('b'), Expr.column('c')]),
      );
      assertParseTarget('SELECT a OR b;', const Expr.or([Expr.column('a'), Expr.column('b')]));
      assertParseTarget('SELECT a OR b OR c;', const Expr.or([Expr.column('a'), Expr.column('b'), Expr.column('c')]));
      assertParseTarget(
        'SELECT a AND b OR c;',
        const Expr.or([
    ...
    test('functions', () {
      assertParseTarget('SELECT random();', const Expr.function('random', []));
      assertParseTarget('SELECT min(a);', const Expr.function('min', [Expr.column('a')]));
      assertParseTarget('SELECT min(a, b);', const Expr.function('min', [Expr.column('a'), Expr.column('b')]));
      assertParseTarget('SELECT count(*);', const Expr.function('count', [Asterisk()]));
    });
    ...
    test('complex expressions', () {
      assertParseTarget(
        ...
        const Expr.not(
          Expr.eq(Expr.column('a'), Expr.neq(Expr.column('b'), Expr.and([Expr.constant(42), Expr.constant(17)]))),
        ),
      );
    });
  });
  ...
  group('precedence', () {
    test('operators', () {
      assertParseTarget(
        ...
        const Expr.or([
          Expr.and([Expr.column('a'), Expr.column('b')]),
          Expr.and([Expr.column('c'), Expr.column('d')]),
        ]),
      );
      assertParseTarget(
        'SELECT a = 2 AND b != 3;',
        const Expr.and([Expr.eq(Expr.column('a'), Expr.constant(2)), Expr.neq(Expr.column('b'), Expr.constant(3))]),
      );
      assertParseTarget('SELECT not a AND b;', const Expr.and([Expr.not(Expr.column('a')), Expr.column('b')]));
      assertParseTarget(
        'SELECT a + b AND c - d;',
        const Expr.and([Expr.add(Expr.column('a'), Expr.column('b')), Expr.sub(Expr.column('c'), Expr.column('d'))]),
      );
      assertParseTarget(
        'SELECT a * b + c / d - 3;',
        const Expr.sub(
  ...
  group('FROM', () {
    final expr = const Expr.eq(
      Expr.column('d'),
      Expr.and([
        Expr.function('max', [Expr.column('e')]),
        Expr.constant(17),
      ]),
    );
    ...
    test('simple from', () {
      assertParseFrom('SELECT a, b FROM d = (max(e) and 17);', FromClause.filter(expression: expr));
    });
    ...
    test('open close clear', () {
      ...
      assertParseFrom(
        ...
        FromClause.filter(expression: expr, close: const CloseSpec.end()),
      );
      assertParseFrom(
      ...
      assertParseFrom('SELECT a, b FROM d = (max(e) and 17) CLEAR;', FromClause.filter(expression: expr, clear: true));
      assertParseFrom(
  ...
  group('FROM and WHERE', () {
    test('both clauses', () {
      ...
      assertParse(
        ...
        select(
          [const Target(expression: Expr.column('a')), const Target(expression: Expr.column('b'))],
          fromClause: FromClause.filter(expression: expr),
          whereClause: expr,
        ),
      );
    });
 ...
 SELECT * FROM date = 2014-05-02
  ...
  group('GROUP BY', () {
    test('columns expressions having and numbers', () {
      assertParse(
        'SELECT * GROUP BY a;',
        select(const Asterisk(), groupBy: const GroupBy(columns: [Expr.column('a')])),
      );
      assertParse(
        'SELECT * GROUP BY a, b, c;',
        select(
          const Asterisk(),
          groupBy: const GroupBy(columns: [Expr.column('a'), Expr.column('b'), Expr.column('c')]),
        ),
      );
      assertParse(
        ...
        select(
          ...
          groupBy: const GroupBy(
            columns: [
              Expr.gt(Expr.function('length', [Expr.column('a')]), Expr.constant(0)),
              Expr.column('b'),
            ],
          ),
        ),
      ...
      assertParse(
        ...
        select(
          const Asterisk(),
          groupBy: const GroupBy(
            columns: [Expr.column('a')],
            having: Expr.eq(Expr.function('sum', [Expr.column('x')]), Expr.constant(0)),
          ),
        ),
      );
      assertParse('SELECT * GROUP BY 1;', select(const Asterisk(), groupBy: const GroupBy(columns: [1])));
      assertParse('SELECT * GROUP BY 2, 4, 5;', select(const Asterisk(), groupBy: const GroupBy(columns: [2, 4, 5])));
  ...
  group('ORDER BY', () {
    test('direction and many columns', () {
      assertParse('SELECT * ORDER BY a;', select(const Asterisk(), orderBy: [const OrderBy(column: Expr.column('a'))]));
      assertParse(
      ...
      assertParse(
        ...
        select(const Asterisk(), orderBy: [const OrderBy(column: Expr.column('a'))]),
      );
      assertParse(
        'SELECT * ORDER BY a DESC;',
        select(
          const Asterisk(),
          orderBy: [const OrderBy(column: Expr.column('a'), ordering: Ordering.desc)],
        ),
      );
      assertParse(
  ...
  group('PIVOT BY', () {
    test('two columns', () {
      ...
      assertParse(
        'SELECT * PIVOT BY a, b',
        select(const Asterisk(), pivotBy: const PivotBy(columns: [Expr.column('a'), Expr.column('b')])),
      );
      assertParse('SELECT * PIVOT BY 1, 2', select(const Asterisk(), pivotBy: const PivotBy(columns: [1, 2])));
    });
  });

  group('options', () {
    test('distinct and limit', () {
      assertParse('SELECT DISTINCT x;', select([const Target(expression: Expr.column('x'))], distinct: true));
  ...
  group('comments', () {
    test('block comments', () {
      assertParse(
        ...
        select([const Target(expression: Expr.column('first')), const Target(expression: Expr.column('second'))]),
      );
      assertParse(
        '''SELECT first, /*
 comment
 */ second;''',
  ...
  group('identifiers and literals', () {
    test('unquoted identifiers fold case', () {
      expect(Query().parse('SELECT Foo;'), select([const Target(expression: Expr.column('foo'))]));
    });

    test('quoted identifiers', () {
      expect(Query().parse('SELECT "foo";'), select([const Target(expression: Expr.constant('foo'))]));
      expect(
        Query().parse('SELECT "foo bar" as "x";'),
        select([const Target(expression: Expr.constant('foo bar'), name: 'x')]),
      );
    });
  ...
  group('CREATE TABLE and INSERT', () {
    test('parse create table', () {
      expect(
        ...
        const CreateTableStatement(name: 'abcd', columns: [('a', 'int'), ('b', 'bool'), ('c', 'str'), ('d', 'date')]),
      );
      expect(
      ...
      expect(
        ...
        CreateTableStatement(
          name: 't',
          query: select([const Target(expression: Expr.constant(1))]),
        ),
      );
    });
    ...
    test('parse insert', () {
      expect(
        ...
        InsertStatement(
          ...
          columns: [const Expr.column('a'), const Expr.column('b'), const Expr.column('c'), const Expr.column('d')],
          values: [
