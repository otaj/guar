...
void main() {
  ...
  test('reports a directive only in the right ledger as onlyInRight', () {
    final ParsedLedger left = parser.parse('2014-01-01 open Assets:Cash\n', filename: 'ledger.beancount');
    final ParsedLedger right = parser.parse(
  ...
  test('same content at different locations matches only when locations are ignored', () {
    const String source = '2014-01-01 open Assets:Cash\n';
    final ParsedLedger left = parser.parse(source, filename: 'a.beancount');
    final ParsedLedger right = parser.parse(source, filename: 'b.beancount');
    expect(parser.diff(left, right, considerLocations: false).isEmpty, isTrue);
    final LedgerDiff located = parser.diff(left, right, considerLocations: true);
  ...
  test('same location with different bodies is changed when locations are considered', () {
    final ParsedLedger left = parser.parse('2014-01-01 open Assets:Cash\n', filename: 'ledger.beancount');
    final ParsedLedger right = parser.parse('2014-01-01 open Assets:Wallet\n', filename: 'ledger.beancount');
    final LedgerDiff located = parser.diff(left, right, considerLocations: true);
    ...
    final LedgerDiff ignored = parser.diff(left, right, considerLocations: false);
  ...
  test('diffs option scalars and list values', () {
    final ParsedLedger left = parser.parse(
    ...
    expect(diff.options.documents.onlyInRight, <String>['/b']);
  ...
  test('plugin locations are ignored unless considerLocations is true', () {
    final ParsedLedger left = parser.parse('plugin "beancount.plugin.unrealized"\n', filename: 'a.beancount');
  ...
  test('diffs parse errors and mixed success versus failure', () {
    final ParsedLedger left = parser.parse('not a directive\n', filename: 'ledger.beancount');
    final ParsedLedger right = parser.parse('also bad\n', filename: 'ledger.beancount');
    final LedgerDiff errors = parser.diff(left, right, considerLocations: false);
    expect(errors.errorsOnlyInLeft, isNotEmpty);
    expect(errors.errorsOnlyInRight, isNotEmpty);

    final ParsedLedger success = parser.parse('2014-01-01 open Assets:Cash\n', filename: 'ledger.beancount');
    final LedgerDiff mixed = parser.diff(success, left, considerLocations: true);
