...
void main() {
  ...
  test('deprecated allow_pipe_separator warns without failing the parse', () {
    final ledger = ok(parser.parse('option "allow_pipe_separator" "TRUE"\n', filename: file));
  ...
  test('deprecated allow_deprecated_none_for_tags_and_links warns', () {
    final ledger = ok(parser.parse('option "allow_deprecated_none_for_tags_and_links" "TRUE"\n', filename: file));
  ...
  test('deprecated inferred_tolerance_multiplier warns', () {
    final ledger = ok(parser.parse('option "inferred_tolerance_multiplier" "1.1"\n', filename: file));
  ...
  test('unsupported insert_pythonpath warns and still records the option', () {
    final ledger = ok(parser.parse('option "insert_pythonpath" "TRUE"\n', filename: file));
  ...
  test('ordinary options do not emit warnings', () {
    final ledger = ok(parser.parse('option "title" "Books"\noption "operating_currency" "USD"\n', filename: file));
...
''';
  ...
  test('splice replaces a warning when the option line is rewritten', () {
    final base = parser.parse('option "allow_pipe_separator" "TRUE"\noption "title" "Books"\n', filename: file);
    expect(ok(base).warnings, hasLength(1));
    final spliced = parser.splice(base, 'option "title" "Books"\n', filename: file, startLine: 1, endLine: 1);
  ...
  test('diff reports warning-only differences', () {
    final left = parser.parse('option "title" "Books"\n', filename: file);
    final right = parser.parse('option "insert_pythonpath" "TRUE"\n', filename: file);
