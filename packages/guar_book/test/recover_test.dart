...
''';
  ...
  test('recovering book keeps the failing balance and the error', () {
    final p.ParsedLedger parsed = const p.BeancountParser().parse(source, filename: 'bal.beancount');
    final Ledger ledger = Book().process(parsed, recover: true);
    expect(ledger, isA<LedgerDirectives>());
    final LedgerDirectives recovered = ledger as LedgerDirectives;
    expect(recovered.directives.where((Directive directive) => directive.body is BalanceBody), hasLength(1));
    expect(recovered.errors.any((ProcessingError error) => error.message.contains('Balance failed')), isTrue);
  });
}
