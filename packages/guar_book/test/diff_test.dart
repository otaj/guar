...
void main() {
  ...
  Ledger bookSource(String source, {String filename = 'ledger.beancount'}) =>
      book.process(parser.parse(source, filename: filename));

  test('identical ledgers produce an empty diff', () {
    final Ledger ledger = bookSource('2014-01-01 open Assets:Cash\n');
    expect(book.diff(ledger, ledger).isEmpty, isTrue);
  });

  test('reports a directive only in the right ledger as onlyInRight', () {
    final Ledger left = bookSource('2014-01-01 open Assets:Cash\n');
    final Ledger right = bookSource('2014-01-01 open Assets:Cash\n2014-01-02 open Assets:Checking\n');
  ...
  test('same content at different locations is equal', () {
    ...
    final Ledger left = bookSource(source, filename: 'a.beancount');
    final Ledger right = bookSource(source, filename: 'b.beancount');
    expect(book.diff(left, right).isEmpty, isTrue);
  });

  test('different bodies at the same span appear as onlyInLeft and onlyInRight', () {
    final Ledger left = bookSource('2014-01-01 open Assets:Cash\n');
    final Ledger right = bookSource('2014-01-01 open Assets:Wallet\n');
    final LedgerDiff diff = book.diff(left, right);
  ...
  test('diffs option scalars after booking defaults are applied', () {
    final Ledger left = bookSource('option "title" "Old"\n2014-01-01 open Assets:Cash\n');
    final Ledger right = bookSource('option "title" "New"\n2014-01-01 open Assets:Cash\n');
    final LedgerDiff diff = book.diff(left, right);
  ...
  test('diffs processing errors and mixed success versus failure', () {
    ...
    final Ledger right = bookSource('2014-01-01 open Assets:Bank\n2014-01-02 open Assets:Bank\n');
    final LedgerDiff errors = book.diff(left, right);
    ...
    final Ledger success = bookSource('2014-01-01 open Assets:Cash\n');
    final LedgerDiff mixed = book.diff(success, left);
