  ...
  test('complete cash transaction passes through with defaults', () {
    final parsed = p.ParsedLedger.directives(
      directives: [
        ...
        p.ParsedDirective(
          location: const p.BeanLocation(linenoBegin: 3, linenoEnd: 5),
          date: const p.BeanDate(year: 2020, month: 2, day: 1),
          body: p.DirectiveBody.transaction(
            p.ParsedTransaction(
              ...
              postings: [
                p.ParsedPosting(
                  location: const p.BeanLocation(linenoBegin: 4, linenoEnd: 4),
                  account: const p.Account(name: 'Expenses:Food'),
                  units: p.IncompleteAmount(
                    number: p.BeanNumber(verbatim: '10.00', resolved: Decimal.parse('10.00')),
                    currency: const p.Currency(name: 'USD'),
                  ),
                ),
                p.ParsedPosting(
                  location: const p.BeanLocation(linenoBegin: 5, linenoEnd: 5),
                  account: const p.Account(name: 'Assets:Cash'),
                  units: p.IncompleteAmount(
                    number: p.BeanNumber(verbatim: '-10.00', resolved: Decimal.parse('-10.00')),
                    currency: const p.Currency(name: 'USD'),
                  ),
  ...
  test('converts total cost to per-unit cost', () {
    final parsed = p.ParsedLedger.directives(
      directives: [
        ...
        p.ParsedDirective(
          location: const p.BeanLocation(linenoBegin: 3, linenoEnd: 5),
          date: const p.BeanDate(year: 2020, month: 3, day: 1),
          body: p.DirectiveBody.transaction(
            p.ParsedTransaction(
              flag: const p.Flag.special(p.SpecialFlag.asterisk),
              narration: 'buy',
              postings: [
                p.ParsedPosting(
                  location: const p.BeanLocation(linenoBegin: 4, linenoEnd: 4),
                  account: const p.Account(name: 'Assets:Shares'),
                  units: p.IncompleteAmount(
                    number: p.BeanNumber(verbatim: '10', resolved: Decimal.fromInt(10)),
                    currency: const p.Currency(name: 'HOOL'),
                  ),
                  cost: p.ParsedCost(
    ...
    expect(txn.postings.first.cost!.date, const BeanDate(year: 2020, month: 3, day: 1));
  });
}
